...
http.createServer(async function (request, response) {
    ...
    try {
        ...
        if (request.url.toLowerCase().startsWith(settings.generic.path.online.api)) { //Als API call
                ...
                if (api[path].enabled.dependencies.installed) {
                    ...
                    if (request.method == 'POST') {
                        ...
                        request.on('end', async function () {
                            ...
                            params = cont;
                            ex.execute((code, text) => { errorCode(response, code, { text: text }); }, parseError, data => { response.end(data) }, request, extra, params, response)
                        })
                    } else {
                        ex.execute((code, text) => { errorCode(response, code, { text: text }); }, parseError, data => { response.end(data) }, request, extra, params, response)
                    }
